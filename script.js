document.addEventListener('DOMContentLoaded', () => {
    ...
    if (THREE && hero3d) {
        ...
        hero3d.innerHTML = '<div class="hero-3d-fallback-inner"><span></span><span></span><span></span></div>';
    }

    const typingName = document.querySelector('.typing-name');
    const nameSpan = document.querySelector('.typing-name span');

    if (typingName && nameSpan) {
        const fullName = nameSpan.textContent.trim() || 'Pidhadiya Diksha';
        ...
        typeLoop().catch((error) => {
            console.error('Type loop error:', error);
        });
    }
