...
function createHeroScene() {
    if (!hero3d || hero3d.dataset.hero3dReady === 'true') {
        return;
    }
    ...
    if (!width || !height) {
        requestAnimationFrame(createHeroScene);
        return;
    }
    ...
    hero3d.dataset.hero3dReady = 'true';

    if ('ResizeObserver' in window) {
        resizeObserver = new ResizeObserver(() => {
            resizeHeroCanvas();
        });
        resizeObserver.observe(hero3d);
    }
...
function animate() {
    if (!renderer || !scene || !camera || !heroGroup || !core || !accentRing || !orbitPath || !particles) {
        return;
    }
...
window.addEventListener('load', createHeroScene, { once: true });
requestAnimationFrame(createHeroScene);
